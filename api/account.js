 ...
 *  	phone:"{string=} 使用人电话号码（格式如：132****1254，type为 1 时可不显示）",
 *  	currentTime:"{string=} 当前时间（即此券使用时间，type为 1 时可不显示）"
 *  }
 * }
 *
 * @needAuth
 ...
 *		days:"{String} 已连续签到天数",
 *		isGet:"{int} 是否获得额外奖励（1--是，0--否）",
 *		remainDays:"{String} 为获得奖励还需连续天数",
 *		extraCoins:"{String} 满足连续天数可获得奖励星币"
 *  }
 * }
 * 
 * @needAuth
 ...
 * @fakedoc xtz.我的体验金
 *
 * @name account.myExperienceMoney
 * @href /account/myExperienceMoney
 *
 * @input.post {string} client 		客户端统计参数（common/client）
 * @input.post {string} token 			Token
 *
 ...
 * @description
 *
 * https://localhost:5000/account/myExperienceMoney
 *
 * https://fakeapi.asterlake.cn:5000/account/myExperienceMoney
 */
router.all('/account/myExperienceMoney', function (req, res, next) {
	var start = req.body.start || 0;
	var limit = req.body.limit || 10;
	var records = [];
	var max = 5;
